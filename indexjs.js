var arr = new Array();
arr[0] = new Array("呼吸内科", "消化内科", "心血管内科", "神经内科", "肿瘤科", "内分泌科", "血液内科", "传染病科");
arr[1] = new Array("普外", "泌尿", "神经外（脑外）", "骨科", "口腔外科", "耳鼻喉", "整形", "烧伤", "胸外", "心脏外", "肝胆");
function Select(index) {
    var Keshiobj = document.getElementById("SelectKeshi");
    Keshiobj.innerHTML = "<option>--请选择--</option>";
    var city = arr[index];
    for(i = 0; i<arr[0].length;i++){
        Keshiobj.innerHTML += ("<option>"+arr[index][i]+"</option>");
    }
}
...
function checkRegister()
{
    ...
    {
        if(($("#registerpPassword").val())!=($("#twiceregistepPassword").val()))
        {
            alert("两次输入的密码不一致");
        }
    }
    if(($("#SexMan").prop("checked") == "") && ($("#SexWoman").prop("checked") == ""))
    {
        alert("请选择性别");
    }
